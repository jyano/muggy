UNIVERSE=function(){z(); $GUYS=  []
    ...
    $getGuy=function(un){ var targetGuy=false //  finds a guy({un, bm}) in $GUYS array from a un ( rets false if he aint there) //alpha: can accept ob received on guyUpdates ({un, x, y})//can pass the string or also an ob with 'un' property
        ...
        return targetGuy}
    $sendMyUpdate=  function(){var loc; if( !window['yourBm']){alert('cannut "$sendMyUpdate" because !"yourBm" bm');return }
        if(_test){$l(yourBm.x + ', ' + yourBm.y)}
        ; k.em('myUpdate', loc= {un: _username, x: yourBm.x, y: yourBm.y})}
    $updateGuyForMe=  function(userOb){var guy; if(!userOb){return}
    ...
    $Bub = function (t, x, y) {var g = G(arguments), ct = $Ct()
        ...
        return ct}
    k.on('bub', function(bub){var guy; if( guy = $getGuy(bub.u) ){$Bub(bub.t, bub.x, bub.y)}})
    k.on('updateGuy', function(guy){$updateGuyForMe(guy)})
    $invite = function (toWho) {k.em('sendInvite', {from: _username, toWho: toWho})}
    k.on('inviteAccepted', function (invite) {if (_username == invite.toWho) {$addGuy(invite.from)} })
    ...
    uni.mug(function(b){if(O(b)){yourBm=    b.dg().rC().XY(600).sXY(.4)}
        $GUYS.push({un :_username, bm : yourBm        }) //b.$$( function(){ b.rm(); k.emit('X', _username)})
        page =  $.d('b', 1000, 'auto').A($.br(3))
        page.A(chatMessageInputTextBox = $.ip('...', 'tx').id('textinput'), chatSendButton = $.bt('send', function(){
            $Bub( $('#textinput').V(), '+')}))
        n=0; $.eG('users', function(user){if(user.mug ){var mugInviteBt=$.i(user.mug).WH(100)
            mugInviteBt.$(function(){$invite( user.un )}); page.A(mugInviteBt)}
        })
        _.ev(.1, $sendMyUpdate)})}
old=function(){
    ...
    $Uni=  $UNI= startUniverse = function(un){
        var guy;
        if( !window['uni'] ){ $UNIVERSE() }
        else {$l('uni found. no need to call UNIVERSE')}
        guy = $getGuy( un )
        if( guy ){$l('we already have guy: ' + guy )}
        else { $l('no guy'); $mug(un, function(mug){$Bm( mug,  function( bm ){$addGuyToMe(un, bm)})})}
...
_test=false // *** fix gx.cir and h.cir !!!! ***